...
Question 3 (q3) on the survey asked participants to select an academic department, but this challenge asks you to create a pie chart that displays broad academic areas.  Thus you will need to create an interim dataframe that summarizes department counts.  You will then need to add an academic group column to that dataframe, in order to classify the department in one of our four major groups.  This final classification task is best done by hand, using R's basic editor.
...
# with the fix() function.  This opens a simple text editor which allows data entry and
# modification.  Strangely, the edit() function opens the same editor but changes are not
# permanently saved to the dataframe.
