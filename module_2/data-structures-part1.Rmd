...
> The `fix()` function opens the (Nano) text editor, allowing you to edit individual 
...
> ### *Ref: Data structures and datatypes*
> 
> Data structures and/or datatypes are incredibly important in a programming language. 
> Many times when R raises an error or you are getting unexpected results, the problem 
> can be traced back to the datatype.  For example, a function might be expecting a 
> number as one of its arguments, and you pass it a character string instead. I often 
> use `class()` to see what the underlying datatype is.
...
> ### *Tip: Attaching and detaching dataframes*
> Although the `attach()` function is useful, it can also create issues, especially
> in situations where you forget to `detach()` a dataframe after use.  The problem
> is that R allows you to attach a dataframe more than once.  When this happens,
> you cannot know which dataframe you're working with because the names are the same. 
...
> Note well: matrices can "contain only one datatype."  And because of that, data frames
