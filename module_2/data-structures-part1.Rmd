...
> ### *Ref: R in Action (Section 4.4)*
> Kabocoff discusses the `fix()` function in this section.  It opens the (Nano) text
> editor, allowing you to edit individual cells in a dataframe.  The `edit()` function
> does essentially the same thing, though Kabocoff does not discuss it here.  And finally,
> you can use the `View()` function to examine and filter a dataframe without modifying
> it.  Please note: this function is mixed case with a capital 'V'.  Because R is
> case-sensitive, you'll get an error if you type it as all lower-case.
...
cats.df <- data.frame(coat = c("calico", "black", "tabby"),
                      weight = c(2.1, 5.0, 3.2),
                      likes_string = c(1, 0, 1))
write.csv(cats.df, "feline_data.csv", row.names = FALSE)
```
...
> ### *Ref: R in Action (Section 2.2)*
> 
> Data structures and/or datatypes are incredibly important so read this section of 
> the textbook slowly and carefully.  Many times when R raises an error or you are
> getting unexpected results, the problem can be traced back to the datatype.  For
> example, a function might be expecting a number as one of its arguments, and you pass
> it a character string instead. I often use `class()` to see what the underlying
> datatype is. Kabacoff does not discuss `class()` in this section but presents it later
> in the book and it is covered later in this worksheet.
...
> ### *Ref: R in Action (Section 4.7)* 
> Kabacoff provides some additional information about *type conversion* in 
> section 4.7 (p. 81).  In particular, table 4.5 lists the functions available
> for testing a variable's datatype and converting datatypes.
...
> ### *Ref: R in Action (Section 2.2.4)*
> Kabacoff discusses the `attach()` function in this section.  However, this function
> can create issues, especially in situations where you forget to `detach()` a dataframe
> after use. The problem is that R allows you to attach a dataframe more than once. 
> When this happens, you cannot know which dataframe you're working with as the names
> are the same.
...
> ### *Ref: R in Action (Section 2.2.2)*
> Note well: matrices can "contain only one datatype."  And because of this, data frames
