---
title: RStudio Introduction (Instructional Worksheet)
date: '`r Sys.Date()`'
output:
...
> You must install base R before installing RStudio.  In fact RStudio won't work
> without R installed.
...
> or 2. select "Run Lines" from the "Code" menu, or 3. hit <kbd>Ctrl-Enter</kbd>
> in Windows or Linux or <kbd>Command-Enter</kbd> on OS X. (This shortcut can
> also be seen by hovering the mouse over the button). To run a block of code,
> select it and then `Run`.  If you have modified a line of code within a block
> of code you have just run, there is no need to reselect the section and `Run`,
> you can use the next button along, `Re-run the previous region`. This will run
> the previous code block including the modifications you have made.
...
> you need to use <kbd>Ctrl+C</kbd> instead of <kbd>Esc</kbd> to cancel the
...
> ### Ref: Functions
> So what is a function? As defined by Webopedia, a software function is "a named
...
> because R sources files from your current working directory.  Inevitably,
> students forget where they're at and then get an error message when trying 
> to load a data file.
