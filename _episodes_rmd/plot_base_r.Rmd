...
## Graphs are Icing on the Data Cake
...
## The Basic Plot
This simple line graph displays per capita income for the United States from 1950 to 2007.  The `plot()` function allows programmers to quickly graph a dataset.  In this case, the data plotted in this graph comes from the gapminder.csv file.  So to create this graph, you'll first need to load the data into a dataframe and then select just those rows where country equals the 'United States'.  Actually plotting the subset is fairly straightforward, though you may need to adjust font sizes, line colors, and some other key arguments.
